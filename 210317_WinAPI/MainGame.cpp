...
HRESULT MainGame::Init()
{
	...
	ImageManager::GetSingleton()->AddImage("패배엔딩", "Image/GameOver.bmp", WINSIZE_X, WINSIZE_Y, 1, 1, false);
	...
	ImageManager::GetSingleton()->AddImage("타이머", "Image/timer.bmp", 535, 50, 10, 1, true, RGB(67, 77, 112));
	...
	ImageManager::GetSingleton()->AddImage("엔딩", "Image/strikers_ending.bmp", WINSIZE_X*3, WINSIZE_Y, 3, 1, true, RGB(67, 77, 112));
...
void MainGame::CheckCollision()
{
	...
	//"플레이어 미사일"과 "잡몹"이 출동했다면
	//if (collisionCheck->GetEnemyCollision())
	//{
	//	enemyManager->SetEnemyLife();				//에너미 목숨 줄이자
	//	collisionCheck->SetEnemyCollision(false);
	//}

	//플레이어 Life추가
	if (collisionCheck->GetLifeCollision())
	{
		playerShip->SetPlusPlayerLife();
		collisionCheck->SetLifeCollision(false);
	}

	//플레이어 Power추가
	if (collisionCheck->GetPowerCollision())
	{
		playerShip->SetPowerNum();
		collisionCheck->SetPowerCollision(false);
	}

	//플레이어 Bomb추가
	if (collisionCheck->GetBombCollision())
	{
		playerShip->SetBombNum();
		collisionCheck->SetBombCollision(false);
	}
}
...
void MainGame::ItemMake()
{
	...
	//	vItemManager[0]->SetDropEnemy(ENEMYDROP);
	//	vItemManager[0]->SetIsItem(true);
	//	vItemManager[0]->SetDropPos(bossManager->GetBoss2Pos());
	//	vItemManager[0]->SetRandItem(2);
	//	//randomItem = 1;
	//	oneTimeDrop[1] = true;
	//	//vItemManager[1]->SetIsItem(false);
